...
const login = async (page: Page,  body: { username: string, password: string }) : Promise<void> => {
    try {
        ...
        if(err.name === 'TimeoutError') throw new InvalidCredentialsException('Invalid credentials');
        console.log(err);
    }
